...
def build_excel(df, ville):
    ...
    for b in oriented_sorted:
        ...
        # Cherche le bâtiment dans rows_df pour obtenir cat/lbl
        nom = b['nom']
        cat   = get_cat(nom)
        lbl   = short_name(nom)
    ...
    for row_idx, (_, b) in enumerate(rows_df.iterrows(), 2):
        ...
        # Culture et rayonnement : lus directement depuis le CSV (extrait du GameDesignResponse)
        # Le userscript ne met ces valeurs que si un CultureComponentDTO existe → on leur fait confiance
