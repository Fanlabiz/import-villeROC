...
# Peuplé dynamiquement lors du parsage du GameDesignResponse
DIMS_THRESHOLDS = {}
...
_CULTURE_KEYS = ['CultureSite', 'Evolving', 'Collectable', 'CityHall', 'Monument',
                 'Watchtower', 'Astronomer', 'BanyanTree', 'CommanderTower', 'SchoolV2',
                 'Woodhenge', 'MidnightClock', 'ArchitectsStudio']
...
def is_real_culture(name, range_val):
    """True si le bâtiment produit vraiment de la culture selon le GameDesignResponse.
    range_val doit venir du CultureComponentDTO (f2), pas de f24 du CityDTO.
    On fait confiance au CSV : si range>1 c'est un vrai rayonnement,
    si range==1 c'est probablement f24 (faux positif) sauf pour les CultureSite."""
    is_culture_type = any(k in name for k in _CULTURE_KEYS)
    # CultureSite : toujours orange
    if 'CultureSite' in name:
        return True
    # Evolving/Collectable avec range >= 2 dans le GameDesignResponse
    if is_culture_type and range_val >= 2:
        return True
    # Evolving/Collectable avec range == 1 : probablement vrai
    if is_culture_type and range_val == 1:
        return True
    return False
...
def build_excel(df, ville):
    ...
    # En-têtes colonnes (ligne 1) — colonnes en ordre décroissant (inversion)
    ws_t.cell(1, 1).value = ''
    for c in range(n_cols):
        cell = ws_t.cell(1, c + 2)
        cell.value = n_cols - 1 - c   # inversion : 0 à droite
        cell.font = Font(bold=True, name='Arial', size=8)
        cell.alignment = ctr
        cell.fill = PatternFill('solid', start_color='E8E8E8')

    # Numéros de lignes (colonne A) — lignes en ordre croissant de bas en haut
    ...
    # Inversion du terrain : (0,0) en bas-à-gauche → bas-à-droite dans Excel = ligne 1 du jeu en bas
    # On mappe : ligne_jeu r0 → excel_r = n_rows - r0 + 1 (inversé)
    #            colonne jeu c0 → excel_c = n_cols - c0 (inversé)
    for (r0, c0, w, h, lbl, color) in placed:
        fill = PatternFill('solid', start_color=color)
        # Origine en bas-droite du terrain Excel
        # La case (r0, c0) du jeu est placée à :
        #   excel_r = (n_rows - (r0+h-1)) + 2  → haut du bâtiment en Excel = ligne la plus basse du jeu
        #   excel_c = (n_cols - (c0+w-1)) + 2  → gauche du bâtiment en Excel = colonne la plus à droite du jeu
        excel_r1 = (n_rows - (r0 + h - 1)) + 1
        excel_c1 = (n_cols - (c0 + w - 1)) + 1
        excel_r2 = (n_rows - r0) + 1
        excel_c2 = (n_cols - c0) + 1
    ...
    for row_idx, (_, b) in enumerate(rows_df.iterrows(), 2):
        ...
        # Le rayonnement issu de f24 (= toujours 1) est un faux positif.
        # Le vrai rayonnement vient du CultureComponentDTO (GameDesignResponse) :
        #   - CultureSite : ray du GameDesignResponse (1, 2 ou 3)
        #   - Evolving/Collectable avec culture > 0 : ray du GameDesignResponse
        #   - Tout le reste (fermes, maisons, workshops, casernes) : ray = 0
        if 'CultureSite' in b['Nom_complet']:
            cult, ray = cult_raw, ray_raw
        elif is_real_culture(b['Nom_complet'], ray_raw) and cult_raw > 0:
            cult, ray = cult_raw, ray_raw
        else:
            cult, ray = 0, 0
        # Seuils de culture (depuis f11 du GameDesignResponse, via DIMS_THRESHOLDS)
        thresholds = DIMS_THRESHOLDS.get(b['Nom_complet'], {})
        s25  = thresholds.get(25, '')
        s50  = thresholds.get(50, '')
        s100 = thresholds.get(100, '')
        # Pour les fermes/maisons/workshops, la culture accumulée est dans cult_raw
        if cat in ('Production', 'Habitation', 'Caserne') and cult_raw > 0:
            s25  = thresholds.get(25, '')
            s50  = thresholds.get(50, '')
            s100 = thresholds.get(100, '')
        level = get_level(b['Nom_complet'])
