...
def build_excel(df, ville):
    ...
    for row_idx, (_, b) in enumerate(rows_df.iterrows(), 2):
        ...
        if is_real_culture(nom, ray_raw):
            # Correction DynamicAge : valeurs dynamiques non disponibles dans le GameDesignResponse statique
            # Ces valeurs seront dans le prochain CSV (via f21.f2 du CityDTO)
            if cult_raw == 0 and 'DynamicAge' in nom and 'CultureSite' in nom:
                cult = 0   # sera rempli depuis le prochain CSV
                ray  = ray_raw
            else:
                cult = cult_raw
                ray  = ray_raw
