...
def build_excel(df, ville):
    ...
    # Déduplique par position top-left et vérifie les chevauchements
    placed = []
    occupied = set()  # toutes les cases déjà occupées
    ...
    for _, b in rows_df.iterrows():
        ...
        # Vérifie qu'aucune case du bâtiment n'est déjà occupée
        cells = [(r0+dr, c0+dc) for dr in range(h) for dc in range(w)]
        if any(cell in occupied for cell in cells):
            continue  # chevauchement → ignoré
